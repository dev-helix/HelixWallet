...
class DeeplinkSchemaMatch {
  static hasSchema(schemaString) {
    ...
    return (
      lowercaseString.startsWith('helix:') ||
  ...
  static navigationRouteFor(event, completionHandler) {
    ...
    if (event.url.toLowerCase().startsWith('bluewallet:helix:') || event.url.toLowerCase().startsWith('bluewallet:lightning:')) {
  ...
  static isBitcoinAddress(address) {
    address = address.replace('helix:', '').replace('HELIX:', '').replace('helix=', '').split('?')[0];
  ...
  static isBothBitcoinAndLightning(url) {
    if (url.includes('lightning') && (url.includes('bitcoin') || url.includes('BITCOIN'))) {
      const txInfo = url.split(/(helix:|HELIX:|lightning:|lightning=|bitcoin=)+/);
      let bitcoin;
      let lndInvoice;
      for (const [index, value] of txInfo.entries()) {
        try {
          // Inside try-catch. We dont wan't to  crash in case of an out-of-bounds error.
          if (value.startsWith('helix') || value.startsWith('HELIX')) {
            bitcoin = `helix:${txInfo[index + 1]}`;
  ...
  static bip21decode(uri) {
    return bip21.decode(uri.replace('HELIX:', 'helix:'));
  }
